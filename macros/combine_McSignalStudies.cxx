...
void combine_McSignalStudies()
{
	/////////////////////////////
	// * U S E R * I N P U T * //
	std::string motherDir = "/users/jt15104/local_Analysis_boostedNmssmHiggs/output_McSignalStudies/nmssmCascadeAnalysis_v01/paramCardType_02/version_a/"; // needs the forward slash on the end
	std::vector<std::string> vecHiggsMassToUse = {"70p0", "90p0", "110p0", "130p0"}; // has to match my already defined naming convention
	std::vector<std::string> vecSusyMassToUse = {"1000p0", "1400p0", "1800p0", "2200p0"}; // has to match my already defined naming convention
	std::string tailArgument = "_ratio0p95_splitting1p0_25000events"; // needs the dash at the beginning
	...
	Plotter_add(motherDir, outputDir, vecHiggsMassToUse, vecSusyMassToUse, tailArgument, "leadingHiggsEta", "secondaryHiggsEta", "higgsEtaBothArms", 0.69, 0.88, 0.67, 0.87, false, true);
...
void Plotter_norm(std::string motherDir, std::string outputDir, std::vector<std::string> vecHiggsMassToUse, std::vector<std::string> vecSusyMassToUse, std::string tailArgument, std::string histogramName, std::string baseSaveName, double legXmin, double legXmax, double legYmin, double legYmax, bool normalLoopOrderSusy, bool normalLoopOrderHiggs)
{
