	...
	for(unsigned int iFile=0; iFile<inputFiles_.size(); ++iFile){
	    ...
	    if( inFile ){
			...
			for(ev.toBegin(); !ev.atEnd(); ++ev, ++ievt){
				...
				// M C S I G N A L - A N A L Y S I S (actually all this should belong to another swcript tbh)
				// ------------------------------------------------------------------------------------------------------------//

				// if signal can use genParticles to examine the signal. like i did in the maxcro script.
